...
export async function svgGroupNotice(
  ...
  const { result, contextInfo, groupAvatarBase64, avatarsBase64 = {}, imagesBase64 = {}, enableDarkMode = false, scale = 3.3 } = options
  ...
  const indexBgColor = enableDarkMode ? '#30363d' : '#e8f4fc'
  ...
  function calculateItemHeight(record: GroupNoticeMessageRaw): number {
    ...
    const contentHeight = Math.max(40, lines.length * 26 + 10)
    const hasImages = (record.message.images?.length || 0) > 0
    const imagesHeight = hasImages ? 80 : 0
    return Math.max(140, contentHeight + imagesHeight + 60)
  }
  ...
  for (let i = 0; i < result.records.length; i++) {
    ...
    if (imageCount > 0) {
      ...
      // 图片数量标签
      noticeItems += `<text x="${imgStartX}" y="${imgY + imgSize + 20}" font-size="12" fill="${accentColor}" font-family="${fontFamily}">🖼️ ${imageCount}张图</text>`
    }
  ...
  const totalHeight = currentY + 60  // 内容高度 + 底部边距
  const H = Math.max(500, totalHeight)
