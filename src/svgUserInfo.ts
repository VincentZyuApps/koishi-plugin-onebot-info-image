...
export async function svgUserInfo(
  ...
  const highlightBg = enableDarkMode ? '#1f2937' : '#f3e5f5'  // 使用主题色的高亮背景
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">` +
    ...
    `<rect x="${infoX + 10}" y="${PADDING + 15}" width="${W - infoX - PADDING - 20}" height="${H - PADDING * 2 - 50}" rx="12" fill="${highlightBg}"/>` +
    ...
    rows +
    `<text x="${PADDING + 10}" y="${H - PADDING - 8}" font-size="11" fill="${watermarkColor}" font-family="monospace">generated by koishi-plugin-onebot-info-image, resvg mode</text>` +
